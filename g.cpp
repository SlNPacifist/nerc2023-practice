...
struct segtree {
    ...
    segtree(int n):v(n*4),add(n*4){}

    void build(int a,int b,int k,vector<LL>&A){
        if(b==a+1){
            add[k]=v[k]=A[a];
            return;
        }
    ...
    LL getmax(int l,int r,int a,int b,int k){
        if(l>=b||r<=a) return 0;
        if(l<=a&&r>=b) return v[k];
        int c=(a+b)/2;
        return add[k]+max(getmax(l,r,a,c,k*2+1),getmax(l,r,c,b,k*2+2));
    }

    int firstabove(int l,LL x,int a,int b,int k){
        if(l>=b) return INF;
        if(v[k]<=x) return INF;
        if(b==a+1) return a;
        int c=(a+b)/2;
        x-=add[k];
        int t=firstabove(l,x,a,c,k*2+1);
        if(t!=INF) return t;
        return firstabove(l,x,c,b,k*2+2);
    }

    int lastabove(int r,LL x,int a,int b,int k){
        if(r<=a) return -1;
        if(v[k]<=x) return -1;
        if(b==a+1) return a;
        int c=(a+b)/2;
        x-=add[k];
        int t=lastabove(r,x,c,b,k*2+2);
        if(t!=-1) return t;
        return lastabove(r,x,a,c,k*2+1);
    }

    void add1(int l,int r,int a,int b,int k){
        if(l>=b||r<=a) return;
        if(l<=a&&r>=b){
            add[k]+=1;
            v[k]+=1;
...
int main(int argc, char **argv){
    ...
    segtree T(n);
    T.build(0,n,0,A);
    forn(i,n) ans+=min(T.getmax(0,i+1,0,n,0),T.getmax(i,n,0,n,0))-A[i];
    ...
    forn(qq,tc){
        ...
        LL left=T.getmax(0,a,0,n,0);
        LL mid=T.getmax(a,b,0,n,0);
        LL right=T.getmax(b,n,0,n,0);
        ...
        if(left<=mid){
            int c=T.firstabove(a,left-1,0,n,0);
            assert(c<b);
            ans-=c-a;
        }else{
            int c=T.lastabove(a,mid,0,n,0);
            assert(c>=0);
            ans+=a-c-1;
        }

        if(right<=mid){
            int c=T.lastabove(b,right-1,0,n,0);
            assert(c>=a);
            ans-=b-c-1;
        }else{
            int c=T.firstabove(b,mid,0,n,0);
            assert(c<n);
            ans+=c-b;
        }
